function [obj_val, x_val, y_val] = solve_project_mgt_ldr(C, beta, W, H, a, b, c)

% 
% +ROMETEST\SOLVE_PROJECT_MGT_LDR Helper routine to solve
...
rome_begin;
h = rome_model('Project Management Example - LDR');
...
rome_constraint(z(1:2:end) >= z_LB);
rome_constraint(z(2:2:end) >= z_LB);
...
dy1 = diff(y, [], 1); 
dy2 = diff(y, [], 2);
rome_constraint([dy1(:); dy2(:)] - w == b + a * (1 - x) .* z);
